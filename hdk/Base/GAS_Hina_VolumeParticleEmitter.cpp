...
fpreal _compute_mass(fpreal TargetSpacing, fpreal TargetDensity, fpreal KernelRadius)
{
	...
	fpreal UnivMass = TargetDensity / maxNumberDensity;
	return UnivMass;
}
...
bool GAS_Hina_VolumeParticleEmitter::_solve(SIM_Engine &engine, SIM_Object *obj, SIM_Time time, SIM_Time timestep)
{
	...
	fpreal mass = _compute_mass(particles->getTargetSpacing(), particles->getTargetDensity(), particles->getTargetSpacing() * particles->getKernelRadiusOverTargetSpacing());
