...
std::shared_ptr<fcl::CollisionObjectf> HinaPE::AsFCLCollider(const GU_Detail &gdp, const SIM_Position &position)
{
	...
	fcl::Transform3f tf = fcl::Transform3f::Identity();
	{
		...
		fcl::Matrix3f linear;
		linear << mat[0][0], mat[1][0], mat[2][0],
				mat[0][1], mat[1][1], mat[2][1],
				mat[0][2], mat[1][2], mat[2][2]; // Eigen Matrix is column major, UT_Matrix4D is row major
		tf.linear() = linear;
...
std::shared_ptr<fcl::CollisionObjectf> HinaPE::AsFCLCollider(const GU_Detail *gdp, const SIM_Position *position) { return AsFCLCollider(*gdp, *position); }
std::shared_ptr<fcl::CollisionObjectf> HinaPE::AsFCLCollider(const GU_ConstDetailHandle gdh, const SIM_Position *position) { return AsFCLCollider(*gdh.gdp(), *position); }