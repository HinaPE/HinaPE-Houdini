...
{
	NeighborBuilder.init({&Fluid.x, &StaticBoundary.x}, radius);
}
void HinaPE::DFSPHSolverCPU::Init(const CPUVectorArray &fluid, const CPUVectorArray &static_boundary)
{
	Fluid.x = fluid;
	StaticBoundary.x = static_boundary;
	Fluid.size = fluid.size();
	StaticBoundary.size = static_boundary.size();
	resize(Fluid);
	resize(StaticBoundary);
...
void HinaPE::DFSPHSolverCPU::for_each_neighbor_fluid(const std::function<void(size_t, Vector)> &f) const { NeighborBuilder.for_each_neighbor(0, 0, f); }
void HinaPE::DFSPHSolverCPU::for_each_neighbor_static_boundary(const std::function<void(size_t, Vector)> &f) const { NeighborBuilder.for_each_neighbor(0, 1, f); }

void HinaPE::DFSPHSolverCPU::advect_pos(HinaPE::real dt) { parallel_for(Fluid.size, [&](size_t i) { Fluid.x[i] += Fluid.v[i] * dt; }); }
void HinaPE::DFSPHSolverCPU::advect_vel(HinaPE::real dt) { parallel_for(Fluid.size, [&](size_t i) { Fluid.v[i] += Fluid.f[i] * Fluid.inv_m[i] * dt; }); }
	...
	while ((Fluid.avg_density_adv - FluidRestDensity) > max_error_d && iter < 100)
	{
		...
		parallel_for(Fluid.size, [&](size_t i)
		{
			...
			for_each_neighbor_static_boundary(
					...
					{
						real m_j = StaticBoundary.m[j];
						real rho_j = StaticBoundary.rho[j];
						real kappa_j = 0;
						Vector grad = m_j * (kappa_i / rho_i + kappa_j / rho_j) * Kernel.gradient(x_i - x_j);
						sum += grad;
					});
